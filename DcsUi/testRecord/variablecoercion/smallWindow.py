...
from Agreement.CS.skio.view.header import CheckBoxHeader
...
class searchWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.date['date'] = self.onComboboxActivate()
...
class mySearchWindow(searchWindow):
    ...
    def searchButtonClicked(self):
        self.queryModel.datas = self.onComboboxActivate()
    ...
    # def table_change(self):
    #     row = self.tableView.currentIndex().row()
    #     var_name = self.queryModel.datas[row]['sig_name']
    #     var = PointModel.get(PointModel.sig_name == var_name)
    #     self.var_win = varCoercion(var, self.queryModel, row)
    #     self.var_win.show()